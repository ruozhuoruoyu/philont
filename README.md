...
| **Plan → review → execute, enforced** | Complex tasks aren't winged. For a multi-step job the agent is **mechanism-forced** through a protocol — *draft a plan → review it → execute against it step by step → close with a real outcome* — and a gate (`plan_protocol_gate`) literally blocks it from skipping ahead to acting before a plan exists. Hard proofs go further: in deep reasoning a "proved" claim must survive **independent adversarial reviewers** before the tree accepts it (`deep_explore`). And a long reasoning session can **auto-advance in the background** round-by-round — you don't babysit it; it reports progress and stops itself when solved or stuck. Every closed task is then reflected on — distilling a reusable routing rule or skill, plus an honest failure note when it didn't work. |
...
**Doing the analytic work, not faking it.** The hard part of a real proof is rarely the structure — it's the *quantitative bookkeeping*: tracking orders of growth like `N^{3/2}·(log N)^{-A}` across dozens of steps, and choosing parameters so the error terms balance. That is exactly where an LLM slips. So `deep_explore`'s verification teeth go past a single solver:
...
The honesty mechanism reaches here too: a node claimed *proved* on an order-bound that was never machine-checked gets flagged, so a hand-wave can't pass downstream as a verified lemma. None of this *invents* the missing mathematical idea — it makes the analytic steps trustworthy, fast, and honest about where the real wall is.

**Not just mathematics.** The reasoning tree itself is **domain-agnostic**: the pattern is always *decompose → claim → **verify***, and the only domain-specific piece is the **verification tooth** (today's are formal/mathematical, so it is strongest on math and logic). Swap or add a tooth and the same engine reaches new fields — a **code executor** for algorithm-correctness proofs and counterexample search (reasoning *about* code, not just running it), a simulator for the physical sciences, a property-based / unit-test harness for engineering. Extending `deep_explore` is a matter of **adding a verifier, not rewriting the engine**.

> **A note from the author — a layman, not a mathematician.** I spent about a week pointing `deep_explore` at the **Goldbach conjecture**. It built and pruned a real reasoning tree and closed off many dead ends, but it did **not** produce a breakthrough — and I'm not equipped to judge how close any of it came. (That experience is exactly why `deep_explore` now carries the `magnitude` and `lemmaLookup` teeth — to do the epsilon-bookkeeping it, and any LLM, slips on.) If you're a mathematician (or a researcher in any formal field), I'd genuinely love for you to try philont on real problems and tell me where it helps and where it falls short. That feedback is exactly what would make `deep_explore` better.
