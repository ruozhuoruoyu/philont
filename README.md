...
Concretely, that means: a **5-layer memory** it carries across every session and channel; a **dual-mode deep-reasoning engine** (`deep_explore`) that thinks hard problems through over many turns — a *formal* mode for mathematical proof and a *deliberate* mode for open-ended, evidence-based judgment (decisions, diagnosis, due diligence); **intrinsic drives** that research and self-review while you're away; mechanism-enforced **honesty** and **plan → review → execute** rigor; a **permission matrix + audit log** on every tool call; **MCP** for any external capability; and one process that reaches you on **WeChat, Telegram, a web UI, or a headless CLI**.
...
| **Deep exploration** | Hard problems get the `deep_explore` treatment: a persistent reasoning **tree** that decomposes the question, attacks sub-problems independently, and only commits a claim after it survives **independent adversarial review**. One engine, two modes — **formal** (mathematical proof, machine-checked) and **deliberate** (open-ended judgment settled by **cited evidence**). Before it grinds, a **feasibility gate** flags goals blocked by a known no-go (e.g. the parity problem); it grounds itself in the literature first; and it tells you when a frontier is genuinely stuck instead of faking progress. |
...
| **Plan → review → execute, enforced** | Complex tasks aren't winged. For a multi-step job the agent is **mechanism-forced** through a protocol — *draft a plan → review it → execute against it step by step → close with a real outcome* — and a gate (`plan_protocol_gate`) literally blocks it from skipping ahead to acting before a plan exists. This is the protocol for **changing the world** (deploy, register, send, deliver an artifact): it decomposes into *action steps* verified by completion, and it always closes. Its sibling `deep_explore` is the protocol for **changing what you know** — see [the rule of thumb](#deep-reasoning-one-engine-two-modes) for which one a task should take. Every closed task is then reflected on — distilling a reusable routing rule or skill, plus an honest failure note when it didn't work. |
