...
test('withNoProgressStop:连续无树提交则早停,reason 提交会重置(真干活永不被切)', async () => {
  // 默认 NO_PROGRESS_CAP=10。
  // (a) 一直失败的 pariGp → 第 10 次触发 abort。
  let aborts1 = 0;
  const failPariGp = async () => ({ ok: false, output: '', error: 'PARI/GP error: ...' });
  const w1 = withNoProgressStop(failPariGp, () => { aborts1++; });
  for (let i = 0; i < 9; i++) { await w1.runner('pariGp', {}); assert.equal(w1.stalled.value, false, `iter ${i} 不该早停`); }
  await w1.runner('pariGp', {}); // 第 10 次
  assert.equal(w1.stalled.value, true);
  assert.equal(aborts1, 1);

  // (b) 每隔几步就成功 reason_record → 计数器重置 → 永不早停(真在干活的轮)。
  let aborts2 = 0;
  let i = 0;
  const mixed = async (name: string) => ({ ok: true, output: 'ok' } as { ok: boolean; output: string });
  const w2 = withNoProgressStop(mixed, () => { aborts2++; });
  for (let k = 0; k < 100; k++) {
    // 5 次 pariGp 后来一次 reason_record(成功提交) → 重置
    const name = (k % 6 === 5) ? 'reason_record' : 'pariGp';
    await w2.runner(name, {});
    i++;
  }
  assert.equal(w2.stalled.value, false, '稳定有 reason 提交的轮次不该被早停');
